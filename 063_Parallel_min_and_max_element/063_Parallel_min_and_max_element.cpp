// 063_Parallel_min_and_max_element.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
// Note
// - 저자 코드가 가장 빠름
...
template <typename Iterator, typename F>
auto pprocess_async(Iterator begin, Iterator end, F&& f)
{
	auto size = std::distance(begin, end);
	if (size <= 10000)
	{
...
template <typename Iterator>
auto pmin_async(Iterator begin, Iterator end)
{
	return pprocess_async(begin, end,
		[](auto b, auto e) {return *std::min_element(b, e); });
}

template <typename Iterator>
auto pmax_async(Iterator begin, Iterator end)
{
	return pprocess_async(begin, end,
		[](auto b, auto e) {return *std::max_element(b, e); });
}
...
int main()
{
	...
	std::vector<FunctionExecutionTimeCalculator> function_execution_times(10);
	...
	result.emplace_back("pmin_async", function_execution_times[i++](pmin_async<decltype(std::begin(data))>, std::begin(data), std::end(data)));
	result.emplace_back("pmax_async", function_execution_times[i++](pmax_async<decltype(std::begin(data))>, std::begin(data), std::end(data)));
